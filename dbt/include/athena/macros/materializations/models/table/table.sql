...
{% materialization table, adapter='athena' -%}
  ...
  {%- set force_batch_insert = config.get('force_batch_insert', False) | as_bool -%}
  ...
  {%- if table_type == 'hive' -%}
    ...
    {%- if is_ha and not is_full_refresh_mode and old_relation is not none -%}
      ...
      -- create tmp table
      {%- set query_result = safe_create_table_as(False, tmp_relation, sql, force_batch_insert) -%}
    ...
    {%- else -%}
      ...
      {%- endif -%}
      {%- set query_result = safe_create_table_as(False, target_relation, sql, force_batch_insert) -%}
    {%- endif -%}
